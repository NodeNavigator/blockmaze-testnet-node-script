...
# ============================================
PEERS=(
  "3db20abc688a1e3323e30d0c1ba2a090a5ab8f28@35.166.208.132:26656"
  "81b7ac56d871a7ab916852bdb281a36f461749ed@52.88.190.174:26656"
  "dc17850e484386416b7f965a5900dd5641fe3f99@35.155.11.220:26656"
  "27bbbd652f0d44ea3f66355539409768fd480934@44.241.215.27:26656"
  "4e977256dcef98fbdcfcc8b2f72beeffcb3e5d66@54.179.29.231:26656"
  "73aebdb950fb4c26783b79cc4c20bff27f2fb93e@13.213.175.77:26656"
  "84a3ae333484e63fc46d45fb9aebd105ad01765f@54.251.190.10:26656"
  "a4b11019154312cc5338567de06f5c97aa3cac15@63.180.208.123:26656"
  "ab81b2f6806386401e10aee463602ee79a858ddb@63.182.21.68:26656"
  "8eefaf7f8639ec03c28b60e6572137f72984c354@3.75.129.94:26656"
  "eb7d9cdae030983ddd635f55194bac7949c0b653@54.209.6.111:26656"
  "4ead61a03b714a0bcc312caace86681bd28f4ded@52.54.47.172:26656"
  "ba9bf15b1f0fabb4c47ffa5ae54e5b4f92321377@34.228.209.102:26656"
  "9cf8ecc967f36f6be9fbfdfeeda4af78c09a83b4@44.245.136.251:26656"
  "7626dd3a2cc80c6d41cc3b2a073868d124360e91@44.250.4.59:26656"
)
...
BINARY_URL="https://github.com/NodeNavigator/blockmaze-testnet-node-script/releases/download/v1.1.0/${BINARY}"
echo $BINARY_URL
...
echo "The file $file_path exists."
  ...
  echo "===========================Copy these keys with mnemonics and save it in safe place ==================================="
	...
	# Configure state sync with dynamic trust values
	STATE_SYNC_RPC_SERVERS="https:\/\/tendermint.testnet.stackflow.site,https:\/\/tendermint-arc.testnet.stackflow.site,https:\/\/tendermint-in.testnet.stackflow.site"
	API_BASE="https://bm-api.testnet.stackflow.site"
    
	echo "Fetching latest block height from $API_BASE..."
	latest_height=$(curl -s "$API_BASE/cosmos/base/tendermint/v1beta1/blocks/latest" | jq -r '.block.header.height')
	if [[ -z "$latest_height" || "$latest_height" == "null" ]]; then
	  echo "Failed to fetch latest block height from $API_BASE"
	  exit 1
	fi

	raw_height=$(( latest_height - 2000 ))
	if [ "$raw_height" -lt 0 ]; then
	  raw_height=0
	fi
	base=$(( raw_height / 1000 ))
	remainder=$(( raw_height % 1000 ))
	if [ "$remainder" -ge 500 ]; then
	  base=$(( base + 1 ))
	fi
	trust_height=$(( base * 1000 ))
	echo "Computed trust height: $trust_height"

	echo "Fetching block hash at height $trust_height..."
	trust_hash_b64=$(curl -s "$API_BASE/cosmos/base/tendermint/v1beta1/blocks/$trust_height" | jq -r '.block_id.hash')
	if [[ -z "$trust_hash_b64" || "$trust_hash_b64" == "null" ]]; then
	  echo "Failed to fetch block hash for height $trust_height"
	  exit 1
	fi
	trust_hash=$(echo "$trust_hash_b64" | base64 --decode | xxd -p -u | tr -d '\n')
	echo "Computed trust hash: $trust_hash"

    grep -q -F '[statesync]' "$CONFIG" && sed -i '/\[statesync\]/,/^\[/ s/enable = false/enable = true/' "$CONFIG"
    sed -i 's/rpc_servers = ""/rpc_servers = "'"$STATE_SYNC_RPC_SERVERS"'"/g' "$CONFIG"
    sed -i 's/trust_height = 0/trust_height = "'"$trust_height"'"/g' "$CONFIG"
    sed -i 's/trust_hash = ""/trust_hash = "'"$trust_hash"'"/g' "$CONFIG"
    sed -i 's/trust_period = "112h0m0s"/trust_period = "168h0m0s"/g' "$CONFIG"


	echo "Running unsafe-reset-all for state sync..."
	$BINARY tendermint unsafe-reset-all --home "$HOMEDIR"
    #======================================================================================================================
